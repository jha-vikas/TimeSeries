...
%minimise using fminunc; x(1) = omega; x(2)=alpha; x(3)=beta; x(4) = d
  ...
  x0 = (rand(1,4))';
  [x,fval,info, output, grad, hess] = maxlike(bgarch,x0);
...
%minimise using fminunc; x(1) = omega; x(2)=alpha; x(3)=beta; x(4) = d; x(5) = c
  hT_1;
  eta(1);
  bgarch = @(x)((ones(1,length(hT_1)))*(log((x(1) + x(2)*hT_1.*((eta.^2 + almostzero^2).^0.5 - x(4)*eta) + x(3)*hT_1.^lambda).^(2/lambda)) + ((z1plus - x(4)*znolag).^2)./((x(1) + x(2)*hT_1.*((eta.^2 + almostzero^2).^0.5 - x(4)*eta) + x(3)*hT_1.^lambda).^(2/lambda))));
  x0 = rand(1,5);
...
%minimise using fminunc; x(1) = omega; x(2)=alpha; x(3)=beta; x(4) = d; x(5) = c
  hT_1;
  eta(1);
  bgarch = @(x)((ones(1,length(hT_1)))*(log((x(1) + x(2)*hT_1.*((1+x(5)^2)*(eta.^2) - x(5)*eta.*(eta.^2 + almostzero^2).^0.5) + x(3)*hT_1.^lambda).^(2/lambda)) + ((z1plus - x(4)*znolag).^2)./((x(1) + x(2)*hT_1.*((1+x(5)^2)*(eta.^2) - x(5)*eta.*(eta.^2 + almostzero^2).^0.5) + x(3)*hT_1.^lambda).^(2/lambda))));
  x0 = rand(1,5);
  [x,fval,info, output, grad, hess] = fminunc(bgarch,x0);
  hT_1 = [(x(1) + x(2)*hT_1.*((1+x(5)^2)*(eta.^2) - x(5)*eta.*(eta.^2 + almostzero^2).^0.5) + x(3)*hT_1.^lambda).^(2/lambda)];
  eta = (z1plus-x(4)*znolag)./hT_1;
